...
function avail_search() {
	var date = document.forms["avail_search_form"].elements["dateselmy"].value + document.forms["avail_search_form"].elements["dateseld"].value;
	...
	set_text('avail_result', getXMLDocText('../../Controller/AvailController.php?date=' + date + '&source=' + source + '&dest=' + dest + '&direct=' + direct + '&cabin=' + cabin +   '&carrier=' + carrier +   '&adulto=' + adulto +   '&bebe=' + bebe +   '&nino=' + nino +   '&mayor=' + mayor));
	return false;
}
...
function cancel_pnr() {
	...
	var ticket = document.forms["cancel_form"].elements["ticket"].value.toUpperCase();
	set_text('cancelled_pnr', getXMLDocText('../../Controller/CancelController.php?&pnr=' + pnr + '&ticket=' + ticket));
...
function book_itinerary(booking){
	...
	hide('booking_search');
 	show('issue');
	set_text('issue_result', getXMLDocText('../../View/Issue/issue.php?BookingID='+booking));
...
function reservation(datos) {
	...
	set_text('booking_search', postXMLDocText('../Book/booking.php?', text + params));
	return false;
...
function booking_submit() {
	...
	//alert(params);
	del_segment(segment);
	set_text('booking_search', postXMLDocText('../../Controller/BookController.php?', text + params));
//	hide('booking_search'); 
//	show('booking_result'); 
	return false;
}
