...
include('../Config/propertiesView.php');
		...
		foreach ($xml->OriginDestinationInformation as $odi) {
			...
			foreach ($odi->OriginDestinationOptions->OriginDestinationOption as $odo) {
				...
				foreach ($odo->FlightSegment as $fs) {
					...
					$OriginDestinationOption['FlightSegment'][$x]=$FlightSegment;
...
|		Presentacion de Datos de la disponibilidad
	...
	*/
	public function AirPriceRS($priceModel){
		$request = '<?xml version=\"1.0\" encoding=\"UTF-8\"?>
				...
				//	}
					$request .= '
						...
						<TravelerInfoSummary>
							<AirTravelerAvail>';
					if(($priceModel->getPassengerTypeQuantityAdulto())>=1){
					$request .= '<PassengerTypeQuantity Code=\"ADT\" Quantity=\"'.$priceModel->getPassengerTypeQuantityAdulto().'\"/>';
					}
					if(($priceModel->getPassengerTypeQuantitybebe()) >0){
					$request .= '<PassengerTypeQuantity Code=\"INF\" Quantity=\"'.$priceModel->getPassengerTypeQuantitybebe().'\"/>';
					}
					if(($priceModel->getPassengerTypeQuantityNino())>0){
					$request .= '<PassengerTypeQuantity Code=\"CNN\" Quantity=\"'.$priceModel->getPassengerTypeQuantityNino().'\"/>';
					}
