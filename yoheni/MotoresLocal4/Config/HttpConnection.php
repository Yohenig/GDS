<?php
define('RAIZ', $_SERVER['DOCUMENT_ROOT']."/MotoresLocal");
...
class HttpConnection {
     ...
     * @return string Respuesta generada por el servidor
     */
    public function post($xml,$follow=false,$header=false) {
         set_time_limit(0);
        $xmlModel= new XMLModel();
        $this->init();
        $xml = str_replace('+', '%20', urlencode($xml));
        curl_setopt($this->curl, CURLOPT_POSTFIELDS, "user=" . $xmlModel->getUser() . "&password=" . $xmlModel->getPassword() . "&request=$xml");

 
        //Recorre la Lista de URL que contiene la variable Servers 
            foreach ($xmlModel->getServers() as $s) {
            curl_setopt($this->curl, CURLOPT_URL, $s);
