        ...
        <form action="<?php echo $uri->toString(array('scheme', 'host', 'port', 'path', 'query')); ?>" method="post" id="mcSignupForm_<?php echo $rand;?>" class="mcSignupForm" name="mcSignupForm<?php echo $rand;?>" onsubmit="return false;"><?php
            foreach ($fields as $f) {
                ...
                switch($type) {
	                ...
	                case 'imageurl': ?>
	                    <div>
                            <input type="text" name="fields[<?php echo $tag;?>]" class="master_text submitInt inputbox<?php
            ...
            <?php endif; ?>
            <div>
                <button type="button" class="master_button button mcSignupSubmit" data-id="<?php echo $rand;?>"> <span><?php echo JText::_($params->get('outro-text-2')); ?></span></button>
            </div>
