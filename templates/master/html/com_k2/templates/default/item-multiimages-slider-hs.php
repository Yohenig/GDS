...
$tab_apps        = JFactory::getApplication();
$tab_template   = $tab_apps->getTemplate(true);
$tab_params     = $tab_template->params;

$tab_map=$tab_params->get('tab_maps');
$tab_cal=$tab_params->get('tab_calendar');

?>

<link rel="stylesheet" href="<?php echo $pluginPathSite; ?>/assets/slider/nivo-slider.css" type="text/css" />
<link rel="stylesheet" href="<?php echo $pluginPathSite; ?>/assets/slider/themes/default/default.css" type="text/css" />

<script type="text/javascript" src="<?php echo $pluginPathSite; ?>/assets/slider/jquery.nivo.slider.js"></script>
 <script type="text/javascript" src="<?php echo $this->baseurl; ?>/templates/<?php echo $templateName; ?>/js/calendar.js"></script>
<!-- Google Map -->
...
<div id="k2Container" class="itemView<?php echo ($this->item->featured) ? ' itemIsFeatured' : ''; ?><?php if($this->item->params->get('pageclass_sfx')) echo ' '.$this->item->params->get('pageclass_sfx'); ?>">
	  ...
	  <!-- Location extra fields -->
			...
			<?php if($extraField->value != ''): ?>
			 <?php if($extraField->name == 'Location'): ?>
			  <span class="extra_location"> <?php echo $extraField->value; ?></span>
			<?php endif; ?>
			<?php endif; ?>
			<?php endforeach; ?>
    ...
    <div class="itemHeader_inner2">	
	  ...
	  <!-- Location extra fields -->
	 	<div class="details_price">
			...
			<?php if($extraField->name == 'Room'): ?>
			  <span class="extra_room"> <?php echo $extraField->value; ?></span>
			<?php endif; ?>
			 <?php if($extraField->name == 'Price'): ?>
			  <span class="extra_price"> <?php echo $extraField->value; ?></span>
			<?php endif; ?>
  ...
  <div class="itemTabArea"> 	 
	  <ul class="nav nav-tabs" role="tablist">
		<li role="presentation" class="active"><a href="#img" aria-controls="img" role="tab" data-toggle="tab"><span class="tri-down"></span><?php echo JText::_('K2_ITEM_PHOTOS'); ?></a></li>
		<?php if($tab_map=="1"){?><li role="presentation"><a href="#map" aria-controls="map" role="tab" data-toggle="tab"><span class="tri-down"></span><?php echo JText::_('K2_ITEM_MAP'); ?></a></li><?php } ?>
		<?php if($tab_cal=="1"){?><li role="presentation"><a href="#calendar" aria-controls="calendar" role="tab" data-toggle="tab"><span class="tri-down"></span><?php echo JText::_('K2_ITEM_CALENDAR'); ?></a></li></a></li><?php } ?>
	  </ul>
 <div class="tab-content">	
   <div role="tabpanel" class="tab-pane active" id="img">
		  <div class="itemImageSlider">
			  ...
			  <div class="slider-wrapper theme-default" style="width:<?php echo $this->item->imageWidth; ?>px; height:auto; margin: 0 auto;">
					...
					<div id="k2article-slider" class="nivoSlider">
						...
						while(
						JFile::exists(JPATH_SITE.DS.'media'.DS.'k2'.DS.'items'.DS.'src'.DS.md5("Image".$this->item->id)."_".$filecount.".jpg") ||
						(JFile::exists(JPATH_SITE.DS.'media'.DS.'k2'.DS.'items'.DS.'src'.DS.md5("Image".$this->item->id).".jpg") && $filecount == 1)
						) {
						
							$xlarge = "image".$filecount."XLarge";
							$image_n = "image".$filecount."XSmall";
							$capt_n = "image".$filecount."_caption";
							$cred_n = "image".$filecount."_credits";
						?> 

						<a class="highslide" href="<?php echo $this->item->$xlarge; ?>" title="<?php if(!empty($this->item->$capt_n)) echo K2HelperUtilities::cleanHtml($this->item->$capt_n); else echo K2HelperUtilities::cleanHtml($this->item->title); ?>" onclick="return hs.expand(this, galleryOptions)" rel="<?php if(!empty($this->item->$capt_n)) echo K2HelperUtilities::cleanHtml($this->item->$capt_n); else echo K2HelperUtilities::cleanHtml($this->item->title); ?> <i><?php echo $this->item->$cred_n; ?></i>">

							<img src="<?php echo $this->item->$xlarge; ?>" data-thumb="<?php echo $this->item->$image_n; ?>" alt="<?php if(!empty($this->item->$capt_n)) echo K2HelperUtilities::cleanHtml($this->item->$capt_n); else echo K2HelperUtilities::cleanHtml($this->item->title); ?>" title="<?php if(!empty($this->item->$capt_n)) echo K2HelperUtilities::cleanHtml($this->item->$capt_n); else echo K2HelperUtilities::cleanHtml($this->item->title); ?>" />		
	...
	<div role="tabpanel" class="tab-pane" id="map">
	 <?php if($this->item->params->get('itemExtraFields') && count($this->item->extra_fields)): ?>
	  <!-- MapCordinates extra fields -->
	 	
			<?php foreach ($this->item->extra_fields as $key=>$extraField): ?>
			<?php if($extraField->value != ''): ?>
		
			<?php if($extraField->name == 'Map Coordinate'): ?>
			  <?php $map_cord = $extraField->value; ?>
			<?php endif; ?>
			<?php endif; ?>
			<?php endforeach; ?>
	  ...
	  <script>
			function initialize() {
			  var mapProp = {
				center:new google.maps.LatLng(<?php echo $map_cord; ?>),
				zoom:5,
	...
	<?php } ?>
     ...
     <div role="tabpanel" class="tab-pane" id="calendar">
	  ...
	  <!-- MapCordinates extra fields -->
			...
			<?php if($extraField->name == 'Booked Date'): ?>
			  <?php $booked_date = $extraField->value; ?>
			<?php endif; ?>
			<?php endif; ?>
			<?php endforeach; ?>
	
	  <?php endif; ?>
        <div class="col-sm-12 col-sm-12 col-xs-12 no-padding cal_inner">
			<div class="calendar"></div>
			<div class="calendar-legend">
				<label class="available"><?php echo JText::_('K2_ITEM_CALENDAR_AVAILABLE'); ?></label>
				<label class="unavailable"><?php echo JText::_('K2_ITEM_CALENDAR_UNAVAILABLE'); ?></label>
				<label class="past"><?php echo JText::_('K2_ITEM_CALENDAR_PAST'); ?></label>
			</div>
        </div>
		<script type="text/javascript">
		 ...
		 // calendar panel
            ...
            var unavailable_days = [<?php echo $booked_date; ?>];
            ...
            //jQuery("#select-month").find("[value='" + current_year_month + "']").prop("selected", "selected");
            cal.generateHTML(current_date.getMonth(), (1900 + current_date.getYear()), unavailable_days);
            jQuery(".calendar").html(cal.getHTML());
			});
		</script>	
      </div> 
	 <?php } ?>
  ...
  <div class="itemBody">
	  ...
	  <span class="desc_heading"><?php echo JText::_('K2_ADDITIONAL_INFO'); ?></span>
     <div class="col-md-12 col-sm-12 col-xs-12 item_bodyinner">
	   
      <div class="item_info col-md-5 col-sm-6 col-xs-12 no-padding-left">		
	  ...
	  <div class="itemExtraFields">
	  	...
	  	<ul>
			...
			<li class="<?php echo ($key%2) ? "odd" : "even"; ?> type<?php echo ucfirst($extraField->type); ?> group<?php echo $extraField->group; ?>">
			 <?php if(($extraField->name != 'Hotel Features') AND ($extraField->name != 'Map Coordinate')) : ?>
	  ...
	  <?php endif; ?>
	   ...
	   <?php
	    ...
	    if($this->item->numOfComments>0 && $this->item->params->get('itemComments') && ($this->item->params->get('comments') == '1' || ($this->item->params->get('comments') == '2'))) {
		//print_r($this->item->comments);
	   ?>
		 <div class="item_client col-md-7 col-sm-6 col-xs-12">
		  <div id="owl-client" class="owl-carousel owl-theme">
		    
		    
				<?php foreach ($this->item->comments as $key=>$comment): ?>
				
				<div class="item <?php echo ($key%2) ? "odd" : "even"; echo (!$this->item->created_by_alias && $comment->userID==$this->item->created_by) ? " authorResponse" : ""; echo($comment->published) ? '':' unpublishedComment'; ?>">
                  <div class="client_comment_slider">
					 <p><?php echo $comment->commentText; ?></p>
                       <div class="col-md-12 col-sm-12 col-xs-12 no-padding">
					    <div class="img_author col-md-2 col-sm-2 col-xs-2 no-padding"> 
							<?php if($comment->userImage): ?>
							<img class="img-circle" src="<?php echo $comment->userImage; ?>" alt="<?php echo JFilterOutput::cleanText($comment->userName); ?>" width="<?php echo $this->item->params->get('commenterImgWidth'); ?>" />
							<?php endif; ?>
						</div>
						<div class="name_author col-md-10 col-sm-10 col-xs-10 no-padding">
		...
		<?php } else { ?>
			<div class="item_client col-md-7 col-sm-7 col-xs-12">
  ...
  <div class="itemBody2">
	  <div class="item_desc col-md-12 col-sm-12 col-xs-12 no-padding">
