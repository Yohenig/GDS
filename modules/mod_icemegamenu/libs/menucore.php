...
class IceMenuTree extends JTree
{
	...
	function toXML()
	{
		...
		while($this->_current->hasChildren())
		{
			...
			$this->_buffer .= '<div><div class="ice-megamenu-toggle"><a class="navbar-toggle collapsed" data-toggle="collapse" data-target=".nav-collapse">'.JText::_("ICE_MAIN_MENU").'</a></div><div class="nav-collapse icemegamenu collapse"><ul id="icemegamenu" class="meganizr mzr-slide mzr-responsive">';
	...
	function _getSubmenuByType()
	{
        ...
        $colWidth 	= $this->_current->getParam("colwidth", 'auto');
        $width 		= $this->_current->getParam("width", 'auto');
	...
	function _getLevelXML($depth, $widthParent=0, $cols_widthParent=0, $colsParent=0, $arr_cols=array(), $parentId=0, $countChild=0, $colsWidth = 0)
	{
	   ...
	   if($this->_current->hasChildren()) {$Class_par = "<i class='fa fa-angle-down'></i>";}
       ...
       if($this->_current->id == $this->_currentItemId || $this->_current->id == JRequest::getVar('Itemid'))
		...
		while($this->_current->hasChildren())
		{
            ...
            $width = $this->_current->getParam("width", 'auto');   
			$cols = $this->_current->getParam("cols",1);
			$cols_width = $this->_current->getParam("colwidth", 'auto');				
